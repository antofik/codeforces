...
namespace Codeforces
{
    ...
    public class RedBlackTree<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>
    {
        ...
        public void Remove(TKey key)
        {
            ...
            Console.WriteLine("After {0}", deleted.Parent);

            if (!deleted.Red)
            {
                Console.WriteLine("Deleted {0}. Fixing", child);
        ...
        private void DeleteFixup(Node node, Node parent)
        {
            ...
            {
                Console.WriteLine("Fix {0} {1}", node==null ? "null" : node.Key.ToString(), parent);
                if (node == parent.Left)
                {
                    Console.WriteLine("\tleft {0}", parent);
                    var brother = parent.Right;
                    if (brother.Red)
                    {
                        Console.WriteLine("\tbrother red {0}", parent);
                        brother.Red = false;
                        parent.Red = true;
                        RotateLeft(parent);
                        brother = parent.Right;
                        Console.Write("[RESULT] {0}", parent.Parent);
                    }
                    if ((brother.Left==null || !brother.Left.Red) && (brother.Right==null || !brother.Right.Red))
                    {
                        Console.WriteLine("\tboth brother's children black {0}", parent);
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine("\tbrother's left is red {0}", parent);
                        ...
                        Console.WriteLine("\tbrother's right is red {0}", parent);
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine("\tboth brother's children black");
                        node = parent.Red ? _root : parent;
                        brother.Red = true;
                        parent.Red = false;
                        Console.Write("[RESULT] {0}", parent.Parent);
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine("\tbrother's right is red");
                            if (brother.Right != null)
                                brother.Right.Red = false;
                            brother.Red = true;
                            RotateLeft(brother);
                        ...
                        Console.WriteLine("\tbrother's left is red");
                        brother.Red = parent.Red;
                        parent.Red = false;
                        if (brother.Left != null)
                            brother.Left.Red = false;
