            ...
            else
            {
                ...
                foreach (var test in tests)
                {
                    ...
                    try
                    {
                        ...
                        writer.Close();
                        stream.Flush();
                        var output = Encoding.UTF8.GetString(stream.ToArray()).Replace("\r", "").TrimEnd('\n');
                        var input = File.ReadAllText(string.Format("../../Task{0}/Tests/test{1}.txt", problem, test));
                        var correctOutput = File.ReadAllText(string.Format("../../Task{0}/Results/test{1}.txt", problem, test)).Replace("\r","");
